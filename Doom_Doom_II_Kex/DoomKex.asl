...
startup
{
	...
	vars.Targets = new Dictionary<string, SigScanTarget>
	{
		...
		{ "arrayPointer1", new SigScanTarget(3, "48 8B 05 ?? ?? ?? ?? 48 85 C0 75 06 B8 FF FF FF FF C3 8B 80 ?? ?? ?? ?? C3") },
		{ "arrayPointer2", new SigScanTarget(15, "84 ?? 74 ?? 48 8B ?? E8 ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 89 05 ?? ?? ?? ?? 48") },
		{ "isInGameOffset1", new SigScanTarget(24, "04 75 ?? 44 39 ?? ?? ?? ?? ?? 74 ?? 44 38 ?? ?? ?? ?? ?? 75 ?? 44 38 ?? ?? ?? ?? ?? 75") },
		...
		{ "mapTicOffset1", new SigScanTarget(9, "48 8B 05 ?? ?? ?? ?? 8B 88 ?? ?? ?? ?? 89 8A ?? ?? ?? ?? EB ?? ?? ?? 48 8B CB E8") },
...
// v0.3.0 05-Nov-2025 https://github.com/neesi/autosplitters/tree/main/Doom_Doom_II_Kex
