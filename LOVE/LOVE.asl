...
init
{
	...
	System.Threading.Tasks.Task.Run(async () =>
	{
		...
		while (!token.IsCancellationRequested && vars.Version == "LOVE itch")
		{
			current.RoomName = "";
			vars.RoomName();
			if (current.RoomName != "")
		...
		while (!token.IsCancellationRequested)
		{
			...
			string[] variableStrings = { "playerTimer", /* "gameType", "playerSpawns" */ };
		...
		if (!token.IsCancellationRequested)
		{
			...
			foreach (Tuple<IntPtr, IntPtr, int, IntPtr, IntPtr, int> elementA in vars.PointerBasesFound)
			{
				...
				{
					IntPtr frameAddress = variablesFound.Where(x => x.Item1 == "playerTimer").Select(x => x.Item2).FirstOrDefault();
