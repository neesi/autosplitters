...
init
{
	...
	System.Threading.Tasks.Task.Run(async () =>
	{
		...
		while (!token.IsCancellationRequested)
		{
			...
			string[] variableNames = { "playertime", /* "gamemode", "spawnpoints" */ };
		...
		if (!token.IsCancellationRequested)
		{
			...
			foreach (Tuple<IntPtr, IntPtr, int, IntPtr, IntPtr, int> elementA in vars.PointerBasesFound)
			{
				...
				{
					IntPtr frameAddress = variableAddressesFound.Where(x => x.Item1 == "playertime").Select(x => x.Item2).FirstOrDefault();
