...
init
{
	...
	System.Threading.Tasks.Task.Run(async () =>
	{
		...
		if (is64bit)
		{
			pointerTargets.Add("RoomNum", new SigScanTarget(14, "E8 ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 89 3D ?? ?? ?? ?? 48 ?? ?? 0F"));
			pointerTargets.Add("RoomBase", new SigScanTarget(6, "00 74 0B 48 8B 05 ?? ?? ?? ?? 48"));
			pointerTargets.Add("VariablePage", new SigScanTarget(3, "48 89 ?? ?? ?? ?? ?? 8B ?? ?? ?? ?? ?? FF C0 89 05 ?? ?? ?? ?? 48 ?? ?? ?? 48"));
		}
		else
		{
