...
init
{
	...
	System.Threading.Tasks.Task.Run(async () =>
	{
		...
		string[] variableTargets = { "playertime", /* "spawnpoints", "gamemode" */ };
		...
		if (is64bit)
		{
			...
			signatureTargets.Add("RoomBase", new SigScanTarget(10, "8D 1C C5 00 00 00 00 ?? 8B ?? ?? ?? ?? ?? 48 ?? ?? ?? 48 ?? ?? 74 05"));
		...
		foreach (var element in pointerBasesFound)
		{
			...
			foreach (var list in variableNamesFoundLists)
			{
				...
				if (variableAddressesFoundCount == variableTargetsCount)
				{
					...
					IntPtr frameCountAddress = variableAddressesFound.Where(x => x.Item1 == "playertime").Select(x => x.Item2).FirstOrDefault();
